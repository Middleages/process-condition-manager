...
- [ ] EC6. 엑셀 파라미터 목록을 임포트(미리보기 → 적용)해 레지스트리에 일괄 등록/갱신할 수 있다 (D-17)
...
| P1-D8 | 백본 복사 시 조건 행 범위 | 백본 layer에 다중 조건 행이 있을 때의 복사 범위 (D-16) | **확정**: 전부 복사 + POR 플래그 그대로 유지. 미매칭 layer는 기본 조건 1행 + POR 미지정으로 생성 |
...
### T8. 파라미터 레지스트리 엑셀 임포트 (D-17)

약 200개 파라미터를 관리 UI에서 한 건씩 등록하는 것은 비현실적 — 사용자가 보유한 엑셀 파라미터 목록을 일괄 주입한다. 파라미터는 지속적으로 추가/변경되므로(D-07) 일회성 스크립트가 아니라 **반복 사용 가능한 임포트 기능**으로 만든다.

- 흐름: 엑셀(.xlsx)/CSV 업로드 또는 TSV 붙여넣기 → 컬럼 매핑 확인 → **dry-run 미리보기** (신규 n / 갱신 n / 오류 n — 중복 code, 타입 불일치, choice 옵션 누락) → 적용
- 적용 규칙: `code` 기준 UPSERT — 신규는 insert, 기존은 update (단 **code 불변** 규칙 유지, code 변경 시도는 오류). 파일에 없는 기존 파라미터는 건드리지 않음 (비활성화는 UI에서 명시적으로만)
- 매핑 대상 필드: `code`(필수), `display_name`, `value_type`(number/text/choice), `category`(없으면 자동 생성), `unit`/`min`/`max`, choice 옵션 목록, `description`, `sort_order` — 엑셀에 없는 필드는 기본값 (value_type 기본 text 등)
- 초기 1회 주입: 사용자 엑셀 수령 → 이 기능으로 주입 (기능 완성 전이면 동일 로직의 시드 스크립트로 선주입 후 기능화)
- `features/parameters` 확장 (Phase 0 CRUD 위에 얹음), 임포트 이력은 결과 요약 로그로 남김

산출물: 임포트 API + 관리 UI (업로드→미리보기→적용) + UPSERT/오류 케이스 테스트. **EC6 충족.**
...
1. 사용자 엑셀 파라미터 목록 수령 → T8 임포트로 레지스트리 초기 주입 (이후 작업의 재료)
