...
**완료 기준:**
- [ ] 프로젝트 목록·생성·상세·시트가 독립 URL과 단일 화면 책임을 갖는다
- [ ] 조건표가 브라우저의 남은 폭·높이를 사용하며 Phase 2 편집 계약이 회귀하지 않는다
- [ ] 파라미터 목록이 첫 viewport에 보이고 생성·수정은 접근 가능한 drawer에서 수행된다
- [ ] semantic token과 공통 상태·폼·버튼 규칙이 핵심 화면에 적용된다
- [ ] 1024/1440/1920 브라우저 QA, 키보드 QA, 전체 frontend gate가 통과한다

Phase 2.5 디자인 승인: 2026-07-13. 구현 작업 분해는 이 설계의 사용자 검토 후 별도 실행
계획으로 작성한다.
