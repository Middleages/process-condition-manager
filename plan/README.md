...
- **layer 매칭 규칙** (D-14 파생): 신규 process의 layer ↔ 백본 프로젝트의 layer를 무엇으로 매칭하는지(layer 이름? stepseq? area 조합?) 확정 필요. Phase 1 착수 전 결정.
- **최초 프로젝트 부트스트랩** (D-14 파생): 참조할 백본 프로젝트가 없을 때의 시작 방식 — "백본 없이 빈 조건표 + 엑셀 붙여넣기"로 가정 중, 확인 필요.
