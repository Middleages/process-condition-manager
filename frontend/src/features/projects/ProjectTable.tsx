    ...
    <div className="max-w-full overflow-x-auto rounded-xl border border-border-subtle bg-surface">
      <table className="w-full min-w-[760px] table-fixed text-left text-sm">
        <thead className="bg-canvas text-xs font-semibold uppercase tracking-wide text-muted">
          <tr>
            <th className="w-[34%] px-4 py-2.5" scope="col">
              프로젝트명
            </th>
            <th className="w-[26%] px-4 py-2.5" scope="col">
              Line / Process
            </th>
            <th className="w-[12%] px-4 py-2.5" scope="col">
              상태
            </th>
            <th className="w-[10%] px-4 py-2.5 text-right" scope="col">
              Layer
            </th>
            <th className="w-[12%] px-4 py-2.5 text-right" scope="col">
              Cell
            </th>
            <th className="w-[6%] px-3 py-2.5" scope="col">
        ...
        <tbody>
          {projects.map((project) => (
            <tr key={project.id} className="border-t border-border-subtle hover:bg-canvas/70">
              <td className="px-4 py-2.5 align-middle">
                <Link
                  className="block w-fit max-w-full rounded-sm font-semibold text-ink-950 underline decoration-transparent underline-offset-4 transition-colors hover:text-brand-700 hover:decoration-current"
              ...
              <td className="px-4 py-2.5 text-right font-mono text-xs tabular-nums text-ink-950">
                {project.layer_count}
              </td>
              <td className="px-4 py-2.5 text-right font-mono text-xs tabular-nums text-ink-950">
                {project.cell_count}
