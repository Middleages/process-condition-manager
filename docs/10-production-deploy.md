...
- **API 응답 필드 변경(브레이킹)**
  - 사용자 식별자 표준이 `userid`로 정리되었습니다.
  - `/api/auth/me` 및 인증 연계 응답에서 `username`, `display_name`은 모두 `userid` 기반 값입니다.
  - 기존에 `display_name`을 별도 사람 이름으로 가정한 클라이언트/스크립트는 배포 전 수정이 필요합니다.
