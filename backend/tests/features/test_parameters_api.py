...
async def test_create_and_list_parameter(db_client: AsyncClient) -> None:
    ...
    listed = (await db_client.get("/api/parameters")).json()
...
async def test_deactivate_is_soft_delete(db_client: AsyncClient) -> None:
    ...
    resp = await db_client.post(f"/api/parameters/{created['id']}/deactivate")
    assert resp.status_code == 200
    assert resp.json()["is_active"] is False

    assert (await db_client.get("/api/parameters")).json() == []
    all_params = (
        await db_client.get("/api/parameters", params={"include_inactive": True})
    ).json()
...
async def test_get_missing_parameter_404(db_client: AsyncClient) -> None:
    resp = await db_client.get("/api/parameters/999")
...
async def test_duplicate_category_rejected_409(db_client: AsyncClient) -> None:
    ...
    dup = await db_client.post("/api/parameters/categories", json=payload)
...
async def test_update_category_fields(db_client: AsyncClient) -> None:
    ...
    cat = (
        await db_client.post(
            "/api/parameters/categories", json={"code": "cmp", "display_name": "CMP"}
        )
    ).json()
    resp = await db_client.patch(
        f"/api/parameters/categories/{cat['id']}",
...
async def test_update_category_missing_404(db_client: AsyncClient) -> None:
    resp = await db_client.patch(
        "/api/parameters/categories/999", json={"display_name": "x"}
    )
    assert resp.status_code == 404
...
async def test_update_parameter_all_editable_fields(db_client: AsyncClient) -> None:
    ...
    cat = (
        await db_client.post(
            "/api/parameters/categories", json={"code": "litho", "display_name": "리소"}
        )
    ).json()
    param = (
        await db_client.post(
...
async def test_update_parameter_unknown_category_404(db_client: AsyncClient) -> None:
    ...
    resp = await db_client.patch(
        f"/api/parameters/{param['id']}", json={"category_id": 777}
    )
    assert resp.status_code == 404
...
async def test_csv_import_preview_then_apply_upserts(db_client: AsyncClient) -> None:
    ...
    # 미리보기는 DB를 바꾸지 않는다
    before = (await db_client.get("/api/parameters")).json()
    ...
    params = {p["code"]: p for p in (await db_client.get("/api/parameters")).json()}
    ...
    # category 자동 생성
    categories = {c["code"] for c in (await db_client.get("/api/parameters/categories")).json()}
