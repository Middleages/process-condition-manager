...
## 9. 리스크 및 대응
- 리네이밍 범위가 커서 회귀 위험 증가 → 호환 경로(구 API alias) 운영.
- 기존 product 기반 히스토리 조회 영향 → natural key 기반 조회 API 병행 제공 후 전환.
- 데이터 무결성 제약 전환 시 배포 리스크 → 단계적 적용(호환 -> 강제).

## 10. 오픈 이슈
1. short 모드(부분 레이어 선택) UX를 1차 릴리즈에 포함할지 여부.
2. Backbone 선택 기본값(없음/권장본 자동선택) 정책.
3. 리네이밍 시점의 구 API 지원 기간.
